package global.tv.channel.live.streaming.free.online.social.food.shopping.utils
...
import global.tv.channel.live.streaming.free.online.social.food.shopping.view.listener.bookmarks.OnActivityTouchListener
import java.util.*
...
class RecyclerTouchListener(var act: Activity, recyclerView: RecyclerView) :
    ...
    fun setLongClickable(
        vibrate: Boolean,
        listener: OnRowLongClickListener?
    ...
    fun setIndependentViews(vararg viewIds: Int?): RecyclerTouchListener {
        independentViews = ArrayList(mutableListOf(*viewIds))
        return this
    }

    fun setUnClickableRows(vararg rows: Int?): RecyclerTouchListener {
        unClickableRows = ArrayList(mutableListOf(*rows))
        return this
    }

    fun setIgnoredViewTypes(viewTypes: Collection<Int>): RecyclerTouchListener? {
        ignoredViewTypes.clear()
        ignoredViewTypes.addAll(viewTypes)
        return this
    }
...
//            bgWidth = rView.getWidth();
     ...
     */
                    ...
                    if (swipeable && !swipedRight && swipedLeftProper && touchedPosition != RecyclerView.NO_POSITION && !unSwipeableRows.contains(
                        ...
                        bgVisibleView = null
                        bgVisiblePosition = -1
                    } else if (swipeable && swipedRight && this!!.bgVisible!!) {
                        // cancel
                        animateFG(touchedView, Animation.OPEN, ANIMATION_STANDARD)
                        bgVisible = true
                        bgVisibleView = fgView
                        ...
                        bgVisibleView = null
                        bgVisiblePosition = -1
                    } else if (swipeable && swipedLeft && this.bgVisible!!) {
                        // cancel
                        animateFG(touchedView, Animation.OPEN, ANIMATION_STANDARD)
                        bgVisible = true
                        bgVisibleView = fgView
                        ...
                        } else if (swipeable && this.bgVisible!! && !fgPartialViewClicked) {
                ...
                if (swipeable && isFgSwiping && !unSwipeableRows.contains(touchedPosition)) {
                    ...
                    if (deltaX < touchSlop && !bgVisible!!) {
                        ...
                        if (bgVisible!!) {
                            ...
                            // swipe fg till it reaches original position. If swiped further, nothing happens (stalls at 0)
                            fgView!!.translationX = (if (translateAmount > 0) 0 else translateAmount) as Float
...
//                        }
    ...
    override fun getTouchCoordinates(ev: MotionEvent?) {
        val y = ev!!.rawY.toInt()
        if (swipeable && bgVisible!! && ev.actionMasked == MotionEvent.ACTION_DOWN && y < heightOutsideRView) closeVisibleBG(
            null
        )
    }
